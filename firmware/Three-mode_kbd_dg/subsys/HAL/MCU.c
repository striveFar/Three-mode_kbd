...
tmosEvents HAL_ProcessEvent( tmosTaskID task_id, tmosEvents events )
{
  ...
  /* USB 挂起之后，每隔100ms开启10ms的接收窗口，其余时间睡眠 */
  if ( events & HAL_LOWPOWER_S1_EVENT )
  {
      extern uint8_t is_usb_sus;
      if(is_usb_sus){
          ...
          sleep_flag = false;
          tmos_start_task(halTaskID, HAL_LOWPOWER_S2_EVENT, MS1_TO_SYSTEM_TIME(10));
      }
    return events ^ HAL_LOWPOWER_S1_EVENT;
  }

  if ( events & HAL_LOWPOWER_S2_EVENT )
  {
      RF_Shut();
      sleep_flag = true;
    return events ^ HAL_LOWPOWER_S2_EVENT;
  }
...
void HAL_Init()
{
  ...
  tmos_start_reload_task(halTaskID, HAL_LOWPOWER_S1_EVENT, MS1_TO_SYSTEM_TIME(100));
