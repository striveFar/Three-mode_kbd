            ...
            case UIS_TOKEN_OUT:
                ...
                if(len == 1){
                    rf_data_t usb_leds ={0};
                        ...
                        rf_set_rxrespdata((uint8_t *)&usb_leds, 5, sizeof(usb_leds) - \
                            sizeof(usb_leds.data) + 1);
                }
...
bool DevEPn_IN_Deal(void) {
    ...
    if(report_id == KEYNORMAL_ID) {
        ...
        /* should not be here */
        LOG_DEBUG("report id error: %#x", report_id);
        lwrb_reset(&KEY_buff);
    }
