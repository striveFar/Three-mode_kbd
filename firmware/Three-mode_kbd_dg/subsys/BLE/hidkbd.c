...
uint16 HidEmu_ProcessEvent(uint8 task_id, uint16 events) {
    ...
    if (events & BLE_SEND_DATA_EVT) {
        ...
        if(lwrb_get_full(&KEY_buff)){
            lwrb_read(&KEY_buff, &keylen, 1);
            lwrb_peek(&KEY_buff, 0, keyVal, keylen);
            if(keylen == 8){
                uint8_t state = HidDev_Report( HID_RPT_ID_KEY_IN, HID_REPORT_TYPE_INPUT, keylen, keyVal);
                switch (state){
                case bleNotReady:  //not connet
                case SUCCESS:
                    lwrb_skip(&KEY_buff, keylen);
                    break;
                ...
                uint8_t state = HidDev_Report( HID_RPT_ID_KEYBIT_IN, HID_REPORT_TYPE_INPUT, keylen, keyVal);
                switch (state){
                case bleNotReady:  //not connet
                case SUCCESS:
                    lwrb_skip(&KEY_buff, keylen);
                ...
                uint8_t state = HidDev_Report( HID_RPT_ID_KEYCONSUME_IN, HID_REPORT_TYPE_INPUT, keylen, keyVal);
                switch (state){
                case bleNotReady:  //not connet
                case SUCCESS:
                    lwrb_skip(&KEY_buff, keylen);
            ...
            if(lwrb_get_full(&KEY_buff))
                tmos_start_task(hidEmuTaskId, BLE_SEND_DATA_EVT, MS1_TO_SYSTEM_TIME(10));  //进一步：根据返回值判断是否再次发送
        }
...
//            GAP_ConfigDeviceAddr( ADDRTYPE_STATIC, ownAddr);
    ...
    case GAPROLE_CONNECTED: {
        ...
        LOG_INFO("hidEmuConnHandle: %#x", hidEmuConnHandle);
